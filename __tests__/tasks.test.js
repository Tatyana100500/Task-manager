...
describe('Tasks', () => {
  const firstUserData = getTestData();
  const secondUserData = getTestData();
  let server;
  let firstUser;
  let secondUser;
  let status;
  ...
  beforeAll(async () => {
    ...
    cookies = await prepareData(server, firstUserData);
  });
  ...
  it('Only creator can delete', async () => {
    cookies = await prepareData(server, secondUser);
    const response = await server.inject({
      method: 'DELETE',
      url: `/tasks/${newTask.id}`,
      cookies,
